...
class D3QNet(object):
    ...
    def train_on_minibatch(self, s_batch, a_batch, r_batch, 
                            s_next_batch, d_batch, w_batch): 
        ...
        Q_next_hat = self.model.predict(s_next_batch, batch_size=self.batch_size)
        
        # Adjusting new target Q values based on action and reward
        for i in range(self.batch_size):
            Q_hat[i, a_batch[i]] = r_batch[i]
            if (not d_batch[i]):
                Q_hat[i, a_batch[i]] += (self.discount_factor * 
                                        Q_next_tar[i, np.argmax(Q_next_hat[i])])
        ...
        with tf.GradientTape() as tape:
            ...
            tf_w_batch = tf.convert_to_tensor(w_batch, dtype=tf.float32)
            batch_loss = tf.math.multiply(batch_loss, tf_w_batch)
            #calculating batch loss
            loss = tf.math.reduce_mean(batch_loss)
        
        prio = batch_error.numpy()
        self.prio =  np.clip(prio, a_min=1e-8, a_max=None)
